                      ...
                      uint8_t Ns)
{
...
#ifdef PHY_TX_THREAD
    if(dlsch->harq_processes[harq_pid]->sib1_br_flag == 1)
    {
        Nacc = 1;
    }
#else
    if(dlsch->sib1_br_flag == 1)
    {
        Nacc = 1;
    }
#endif
    else if(dlsch->rnti == 0xFFFF || dlsch->rnti == 0xFFFE)
    {
        Nacc = (frame_parms->frame_type == TDD) ? 10 : 4;
    }
#ifdef PHY_TX_THREAD
    // Note: above SC-RNTI will also have to be added when/if implemented
    else if(dlsch->harq_processes[harq_pid]->CEmode == CEmodeA)
    {
        Nacc = 1;
    }
    else if(dlsch->harq_processes[harq_pid]->CEmode == CEmodeB)
    {
        Nacc = (frame_parms->frame_type == TDD) ? 10 : 4;
    }
#else
    // Note: above SC-RNTI will also have to be added when/if implemented
    else if(dlsch->CEmode == CEmodeA)
    {
        Nacc = 1;
    }
    else if(dlsch->CEmode == CEmodeB)
    {
        Nacc = (frame_parms->frame_type == TDD) ? 10 : 4;
    }
#endif
    ...
    if(mbsfn_flag == 0)
    {
        ...
        if(dlsch->i0 != 0xFFFF)
        {
            ...
            if((frame & 1023) < 200)
            {
                LOG_D(PHY, "Scrambling init for (i0 %d, i %d, j0 %d, j %d, Nacc %d) => x2 %d\n", i0, i, j0, j, Nacc, x2);
            }
        }
        else
#endif
