              ...
              )
{
    ...
    if(eNB)
    {
...
#endif
        {
            ...
            if(LOG_DEBUGFLAG(PRACH))
            {
                if(((eNB->proc.frame_prach) & 1023) < 20)
                {
                    LOG_I(PHY, "PRACH (eNB) : running rx_prach for subframe %d, prach_FreqOffset %d, prach_ConfigIndex %d , rootSequenceIndex %d\n", subframe, fp->prach_config_common.prach_ConfigInfo.prach_FreqOffset, prach_ConfigIndex, rootSequenceIndex);
                }
            }
        }
    }
        ...
        if(ru->if_south == LOCAL_RF)    // set the time-domain signal if we have to use it in this node
        {
            ...
            if(LOG_DUMPFLAG(PRACH))
            {
                ...
                if(dB_fixed(en0) > 32)
                {
                    ...
                    if(prach[0] != NULL)
                    {
                        LOG_M("prach_rx", "prach_rx", prach[0], fp->samples_per_tti, 1, 1);
                    }
    ...
    else if(eNB != NULL)
    {
        if(LOG_DEBUGFLAG(PRACH))
        {
            ...
            if((en > 60) && (br_flag == 1))
            {
                LOG_I(PHY, "PRACH (br_flag %d,ce_level %d, n_ra_prb %d, k %d): Frame %d, Subframe %d => %d dB\n", br_flag, ce_level, n_ra_prb, k, eNB->proc.frame_rx, eNB->proc.subframe_rx, en);
            }
        }
    }
    ...
    *avg_preamble_energy = 0;
    ...
    for(preamble_index = 0 ; preamble_index < 64 ; preamble_index++)
    {
        if(LOG_DEBUGFLAG(PRACH))
        {
            ...
            if(en > 60)
            {
                LOG_I(PHY, "frame %d, subframe %d : Trying preamble %d (br_flag %d)\n", ru->proc.frame_prach, subframe, preamble_index, br_flag);
            }
        }
        ...
        if(new_dft == 1)
        {
            ...
            if(br_flag == 1)
            {
                ...
                if(eNB->prach_vars_br.repetition_number[ce_level] == 1)
                {
            ...
            if(LOG_DUMPFLAG(PRACH))
            {
                if(prach[0] != NULL)
                {
                    LOG_M("prach_rx0.m", "prach_rx0", prach[0], 6144 + 792, 1, 1);
                }
...
#endif
        {
            if(LOG_DEBUGFLAG(PRACH))
            {
                ...
                if(en > 60)
                {
                    LOG_I(PHY, "frame %d, subframe %d: Checking for peak in time-domain (br_flag %d, en %d)\n", ru->proc.frame_prach, subframe, br_flag, en);
                }
            }
    ...
    else     // This is procedure for eMTC, basically handling the repetitions
    {
        ...
        for(i = 0; i < 4; i++)
        {
                    ...
                    ((prach_mask & (1 << (i + 1))) > 0)) // check that prach CE level is active now
            {
                ...
                {
                    eNB->prach_vars_br.first_frame[i] = eNB->proc.frame_prach_br;
                }
