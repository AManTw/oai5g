                           ...
                           int do_meas)
{
    ...
    if(do_meas == 1)
    {
        stop_meas(&eNB->dlsch_common_and_dci);
    }

    if(do_meas == 1)
    {
        start_meas(&eNB->dlsch_ue_specific);
    }
    ...
    if(do_meas == 1)
    {
        stop_meas(&eNB->dlsch_ue_specific);
    }
    ...
    for(m = 0; m < M; m++)
    {
        ...
        if(harq_pid >= 0 && harq_pid < dlsch0->Mdlharq)
        {
            ...
            if(dlsch0_harq->status == ACTIVE ||
                    (dlsch1_harq != NULL && dlsch1_harq->status == ACTIVE))
            {
                Mtx ++;
            }
        }
    ...
    else                          // TDD
    {
        ...
        for(i = 0; i < ulsch_harq->O_ACK; i++)
        {
            ...
            */
            ...
            {
                release_harq(eNB, UE_id, 0, frame, subframe, 0xffff, ulsch_harq->o_ACK[i] == 1);
            }
