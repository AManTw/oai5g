                            ...
                            uint8_t *sdu)
{
    ...
    if((rel13->pdsch_payload_type < 2) && (rel13->ue_type > 0))
    {
        dlsch0->harq_ids[proc->frame_tx % 2][proc->subframe_tx] = 0;
    }
...
# else
    ...
    if((rel13->pdsch_payload_type < 2) && (rel13->ue_type > 0)) // this is a BR/CE UE and SIB1-BR/SI-BR
    {
        ...
        if(rel13->pdsch_payload_type == 0)
        {
            dlsch0_harq->sib1_br_flag = 1;
        }
...
void handle_ulsch_cqi_ri_pdu(PHY_VARS_eNB *eNB, int UE_id, nfapi_ul_config_request_pdu_t *ul_config_pdu, uint16_t frame, uint8_t subframe)
{
    ...
    {
        ulsch_harq->Or2 = rel9->aperiodic_cqi_pmi_ri_report.cc[0].dl_cqi_pmi_size[1];
    }
