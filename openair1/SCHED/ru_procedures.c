...
static void *fep_thread(void *param)
{
    ...
    while(!oai_exit)
    {
        ...
        if(wait_on_condition(&proc->mutex_fep, &proc->cond_fep, &proc->instance_cnt_fep, "fep thread") < 0)
        {
            break;
        }
        if(oai_exit)
        ...
        if(release_thread(&proc->mutex_fep, &proc->instance_cnt_fep, "fep thread") < 0)
        {
            break;
        }
