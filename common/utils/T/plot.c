...
static void *plot_thread(void *_p)
{
    ...
    while(1)
    {
        ...
        {
            char b[512];
            if(read(p->timer_pipe[0], b, 512) <= 0)
            {
                abort();
            }
            replot = 1;
...
void iq_plot_add_iq_point_loop(void *_plot, short i, short q, int pp)
{
    ...
    if(p->p[pp].iq_count != p->p[pp].count)
    {
        p->p[pp].iq_count++;
    }
    p->p[pp].iq_insert_pos++;
    if(p->p[pp].iq_insert_pos == p->p[pp].count)
    {
        p->p[pp].iq_insert_pos = 0;
    }
    if(pthread_mutex_unlock(&p->lock))
    {
        abort();
