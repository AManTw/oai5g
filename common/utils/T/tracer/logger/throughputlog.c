                          ...
                          char *event_name, char *data_varname)
{
    ...
    for(i = 0; i < f.count; i++)
    {
        if(!strcmp(f.name[i], frame_varname))
        {
            ret->tick_frame_arg = i;
        }
        if(!strcmp(f.name[i], subframe_varname))
        {
            ret->tick_subframe_arg = i;
        }
    }
    ...
    for(i = 0; i < f.count; i++)
    {
        if(!strcmp(f.name[i], frame_varname))
        {
            ret->data_frame_arg = i;
        }
        if(!strcmp(f.name[i], subframe_varname))
        {
            ret->data_subframe_arg = i;
        }
        if(!strcmp(f.name[i], data_varname))
        {
            ret->data_arg = i;
        }
    }
