...
void unlock(void)
{
    if(pthread_mutex_unlock(&biglock))
    {
        abort();
    }
}
...
void get_message(int s)
{
    ...
    switch(t)
    {
        case 0:
            ...
            if(read(s, &l, sizeof(int)) != sizeof(int))
            {
                QUIT("get_message fails");
            }
            while(l)
            {
                if(read(s, &id, sizeof(int)) != sizeof(int))
                {
                    QUIT("get_message fails");
                }
                if(id > is_on_size - 1)
...
int main(int n, char **v)
{
    ...
    while(1)
    {
        ...
        if(fread(v + vpos, sizeof(struct timespec), 1, in) != 1)
        {
            ERR;
            break;
        }
        ...
        vpos += sizeof(int);
        length -= sizeof(int);
        if(length) if(fread(v + vpos, length, 1, in) != 1)
            {
                ERR;
                break;
            }
        ...
        {
            if(type > is_on_size - 1)
            {
                set_is_on_size(type + 1);
            }
            do_send = is_on[type];
        }
        unlock();
