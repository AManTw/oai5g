...
{
name: idname
}, r->i, r->isize, sizeof(id), id_cmp) != NULL)
    {
    ...
    return (id *)bsearch(&(id)
    {
name: idname
    }, r->i, r->isize, sizeof(id), id_cmp);
}
...
void add_groups(database *r, id *i, char *groups)
{
    ...
    if(i == NULL)
    {
        printf("ERROR: GROUP line before ID line\n");
        exit(1);
    }
    while(1)
    {
...
void add_desc(id *i, char *desc)
{
    if(i == NULL)
    {
        printf("ERROR: DESC line before ID line\n");
        exit(1);
    }
...
void add_vcd_name(id *i, char *vcd_name)
{
    if(i == NULL)
    {
        printf("ERROR: VCD_NAME line before ID line\n");
        exit(1);
    }
...
void *parse_database(char *filename)
{
    ...
    in = fopen(filename, "r"); if(in == NULL)
    {
        perror(filename);
        abort();
    }
    ...
    while(1)
    {
        ...
        if(!strcmp(name, "ID"))
        {
            last_id = add_id(r, value, i);
            i++;
        }
        if(!strcmp(name, "GROUP"))
        {
            add_groups(r, last_id, value);
        }
        if(!strcmp(name, "DESC"))
        {
            add_desc(last_id, value);
        }
        if(!strcmp(name, "FORMAT"))
        {
            add_format(last_id, value);
        }
        if(!strcmp(name, "VCD_NAME"))
        {
            add_vcd_name(last_id, value);
        }
    }
...
static int onoff_group(database *d, char *name, int *a, int onoff)
{
    ...
    {
name: name
    }, d->g, d->gsize, sizeof(group), group_cmp);
    if(g == NULL)
    {
        return 0;
    }
