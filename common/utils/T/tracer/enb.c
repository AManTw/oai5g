...
no_connection:
    if(pthread_mutex_unlock(&d->lock))
    {
        abort();
    }
    return;
                  ...
                  char *notification, widget *w, void *notification_data)
{
    ...
    if(w == e->next_ue_button)
    {
        ue++;
    }
    if(w == e->current_ue_button)
    {
        do_reset = 1;
    }

    if(pthread_mutex_lock(&ed->lock))
    {
        abort();
    }
    if(do_reset)
    ...
    if(pthread_mutex_unlock(&ed->lock))
    {
        abort();
    }
}
...
int main(int n, char **v)
{
    ...
    if(pthread_mutex_init(&enb_data.lock, NULL))
    {
        abort();
    }
...
restart:
    clear_remote_config();
    if(enb_data.socket != -1)
    {
        close(enb_data.socket);
    }
    ...
    while(1)
    {
        ...
        if(pthread_mutex_lock(&enb_data.lock))
        {
            abort();
        }
        handle_event(h, e);
        if(pthread_mutex_unlock(&enb_data.lock))
        {
            abort();
        }
    }
