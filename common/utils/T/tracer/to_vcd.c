...
int main(int n, char **v)
{
    ...
    {
        char format[256];
        if(strlen(vars[i].arg) + strlen(vars[i].vcd_name) > 256 - 16)
        {
            abort();
        }
    ...
    while(run)
    {
        ...
        if(e.type == -1)
        {
            printf("disconnected? let's quit gently\n");
            break;
        }
        handle_event(h, e);
    }
