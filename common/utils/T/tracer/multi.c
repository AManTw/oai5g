...
addr: (src), length: (send_size)
        ...
        if(socket_send(s, &T_LOCAL_size, 4) == -1)
        {
            return -1;
        }
        if(socket_send(s, buf, T_LOCAL_size) == -1)
        {
            return -1;
        }
        src += send_size;
...
int send_is_on(int socket, int number_of_events, int *is_on)
{
    ...
    {
        int v = is_on[i] ? 1 : 0;
        if(socket_send(socket, &v, sizeof(int)) == -1)
        {
            return -1;
        }
    }
...
int read_tracee(int s, OBUF *ebuf, int *_type, int32_t *_length)
{
    ...
    vpos += 4;
#ifdef T_SEND_TIME
    if(fullread(s, v + vpos, sizeof(struct timespec)) == -1)
    {
        return -1;
    }
...
#endif
    if(fullread(s, &type, sizeof(int)) == -1)
    {
        return -1;
    }
...
int main(int n, char **v)
{
    ...
    while(1)
    {
        ...
        if(s == -1)
        {
            s = connect_to_tracee(ip, port, number_of_events, is_on);
        }
        ...
        if(s != -1 && fds[1].revents & POLLIN)
        {
            ...
            else
            {
                ...
                {
                    forward_event(&m, number_of_events, &ebuf, type);
                }
            }
        }
        ...
        for(j = 0; j < m.ti_size; j++)
        {
            ...
            if(fds[i].revents & POLLIN)
            {
                ...
                switch(t)
                {
                    case 0:
                        is_on_changed = 1;
                        if(fullread(s, &len, sizeof(int)) == -1)
                        {
                            goto tracer_error;
                        }
                        for(l = 0; l < len; l++)
                        {
                            if(fullread(s, &v, sizeof(int)) == -1)
                            {
                                goto tracer_error;
                            }
                            if(v < 0 || v >= number_of_events)
                            {
                                goto tracer_error;
                            }
