                          ...
                          struct timespec start, struct timespec end)
{
    ...
    while(b >= a)
    {
        ...
        if(time_cmp(this->p[sub].tick[i], start) < 0)
        {
            a = mid + 1;
        }
        else if(time_cmp(this->p[sub].tick[i], end) > 0)
        {
            b = mid - 1;
        }
        else
                   ...
                   char *notification, widget *w, void *notification_data)
{
    ...
    if(!strcmp(notification, "scrollup"))
    {
        mul = 1 / mul;
    }
...
static void append(view *_this, struct timespec t, int frame, int subframe)
{
    ...
    if(pthread_mutex_lock(&ticktime->lock))
    {
        abort();
    }
    ...
    for(i = p->ticksize - 2; i >= 0; i--)
    {
        ...
        if(time_cmp(p->tick[prev], p->tick[cur]) <= 0)
        {
            break;
        }
        swap = p->tick[prev];
    ...
    if(pthread_mutex_unlock(&ticktime->lock))
    {
        abort();
    }
}
...
view *new_subview_ticktime(view *_time, int line, int color, int size)
{
    ...
    if(ticktime->p[ticktime->psize].tick == NULL)
    {
        abort();
    }
                         ...
                         int frame, int subframe)
{
    ...
    */
    ...
    if(pthread_mutex_unlock(&tt->lock))
    {
        abort();
    }
}
