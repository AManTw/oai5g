...
static void del_child(gui *g, widget *_this, widget *child)
{
    ...
    this->growable = realloc(this->growable, (this->nchildren - 1) * sizeof(int));
    if(this->nchildren != 1 && this->growable == NULL)
    {
        abort();
    }
...
hints_ok:
    ...
    {
        int ngrowable = 0;
        for(i = 0; i < this->nchildren; i++) if(this->growable[i])
            {
                ngrowable++;
            }
        if(ngrowable)
