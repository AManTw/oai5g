...
static GC create_gc(Display *d, char *color)
{
    ...
    {
        ERR("X: could not allocate color '%s'\n", color);
    }
...
x_connection *x_open(void)
{
    ...
    if(ret->d == NULL)
    {
        ERR("error calling XOpenDisplay: no X? you root?\n");
    }
        ...
        switch(ev.type)
        {
            ...
            case ButtonPress:
                if((w = find_x_window(g, ev.xbutton.window)) != NULL)
                {
                    ...
                    {
                        key_modifiers |= KEY_SHIFT;
                    }

                    if(ev.xbutton.state & Mod1Mask)
                    {
                        key_modifiers |= KEY_ALT;
                    }

                    if(ev.xbutton.state & ControlMask)
                    {
                        key_modifiers |= KEY_CONTROL;
                    }
                           ...
                           int clipx, int clipy, int clipwidth, int clipheight)
{
    ...
    if(XftDrawSetClipRectangles(w->xft, 0, 0, &clip, 1) == False)
    {
        abort();
    }
    ...
    if(XftDrawSetClip(w->xft, NULL) == False)
    {
        abort();
    }
}
