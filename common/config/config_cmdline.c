...
int config_check_unknown_cmdlineopt(char *prefix)
{
    ...
    {
        if(!finalcheck && strstr(config_get_if()->argv[i], testprefix) == NULL)
        {
            continue;
        }

        if(!finalcheck && testprefix[0] == 0 && index(config_get_if()->argv[i], '.') != NULL)
        {
            continue;
        }

        if(!finalcheck && isdigit(config_get_if()->argv[i][0]))
        {
            continue;
        }

        if(!finalcheck && config_get_if()->argv[i][0] == '-' && isdigit(config_get_if()->argv[i][1]))
        {
            continue;
        }
