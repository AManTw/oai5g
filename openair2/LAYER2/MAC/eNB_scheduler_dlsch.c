...
//------------------------------------------------------------------------------
{
    ...
    for(CC_id = 0; CC_id < nb_mac_CC; ++CC_id)
    {
        ...
        for(i = 0; i < sli->n_dl; ++i)
        {
            ...
            if(has_traffic[CC_id][slice_idx] == 0)
            {
                continue;
            }
            ...
            for(UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id])
            {
                ...
                for(rbg = 0; rbg < N_RBG[CC_id]; ++rbg)
                {
                    ...
                    if(allocation_mask[CC_id][rbg] == 0)
                    {
                        continue;
                    }

                    if(rballoc_sub[CC_id][rbg] != 0)
                    {
                        continue;
                    }

                    if(ue_sched_ctl->rballoc_sub_UE[CC_id][rbg] != 0)
                    {
                        continue;
                    }
...
//------------------------------------------------------------------------------
{
    ...
    for(UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id])
    {
        ...
        if(subframeP == 5)    // MPDCCH
        {
            if(round_DL < 8)
            {
                LOG_D(MAC, "MPDCCH round_DL = %d in frame %d subframe %d\n", round_DL, frameP, subframeP);
            }
