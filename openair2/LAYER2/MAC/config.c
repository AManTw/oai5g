                          ...
                          )
{
    ...
    if(mib != NULL)
    {
        ...
        if(ul_CarrierFreq > 0)
        {
            RC.mac[Mod_idP]->common_channels[CC_idP].ul_CarrierFreq = ul_CarrierFreq;
        }

        if(ul_Bandwidth)
        {
            RC.mac[Mod_idP]->common_channels[CC_idP].ul_Bandwidth = *ul_Bandwidth;
        }
        else
        {
            RC.mac[Mod_idP]->common_channels[CC_idP].ul_Bandwidth = RC.mac[Mod_idP]->common_channels[CC_idP].mib->message.dl_Bandwidth;
        }
