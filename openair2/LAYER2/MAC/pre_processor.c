...
void sort_UEs(module_id_t Mod_idP, int slice_idx, int frameP, sub_frame_t subframeP)
{
    ...
    for(i = 0; i < MAX_MOBILES_PER_ENB; i++)
    {
        ...
        if(UE_RNTI(Mod_idP, i) == NOT_A_RNTI)
        {
            continue;
        }
        ...
        if(!ue_dl_slice_membership(Mod_idP, i, slice_idx))
        {
            continue;
        }
        ...
        const uint8_t rbs_retx[NFAPI_CC_MAX])
{
    ...
    for(UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id])
    {
        ...
        if(UE_RNTI(Mod_id, UE_id) == NOT_A_RNTI)
        {
            continue;
        }
...
static int ue_ul_compare(const void *_a, const void *_b, void *_params)
{
    ...
    if(bytes_to_schedule1 < 0)
    {
        bytes_to_schedule1 = 0;
    }
    ...
    if(bytes_to_schedule2 < 0)
    {
        bytes_to_schedule2 = 0;
    }
