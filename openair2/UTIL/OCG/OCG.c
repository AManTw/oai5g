    ...
    while(state != STATE_END)
    {
        ...
        switch(state)
        {
            ...
            case STATE_DETECT_FILE :
                ...
                else if(detect_file_OK == MODULE_ERROR)
                {
            ...
            case STATE_PARSE_FILENAME :
                ...
                if((parse_filename_OK = parse_filename(filename)) == MODULE_OK)
                {
                    state = STATE_CREATE_DIR;
                }
                else
                {
                    if(copy_or_move == 2)
            ...
            case STATE_PARSE_XML :
                if((parse_XML_OK = parse_XML(src_file)) == MODULE_OK)
                {
                    state = STATE_SAVE_XML;
                }
                else
                {
                    if(copy_or_move == 2)
