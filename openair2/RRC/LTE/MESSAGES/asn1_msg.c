...
uint8_t do_MIB_FeMBMS(rrc_eNB_carrier_data_t *carrier, uint32_t N_RB_DL, uint32_t additionalNonMBSFN, uint32_t frame)
{
    ...
    if(spare == NULL || additionalNonMBSFNSubframes == NULL)
    {
        abort();
    }
...
//-----------------------------------------------------------------------------
{
    ...
    if(UEcap)
    {
        ...
        if(featureGroupIndicators)
        {
            ...
            {
                LOG_W(RRC, "[do_DrxConfig] Not enough featureGroupIndicators bits\n");
            }
        }
        else
        {
            LOG_W(RRC, "[do_DrxConfig] No featureGroupIndicators pointer\n");
        }
    }
    else
    {
        LOG_W(RRC, "[do_DrxConfig] No UEcap pointer\n");
    }
