                ...
                if(CCsParamList.numelt > 0)
                {
                    ...
                    for(j = 0; j < CCsParamList.numelt ; j++)
                    {
                        ...
                        if(!NODE_IS_DU(rrc->node_type))    //this is CU or eNB, SIB2-20 in CU
                        {
                            ...
                            {
                                fill_eMTC_configuration(msg_p, &eMTCconfig, i, j, RC.config_file_name, brparamspath);
                            }
                            else
                            {
                            ...
                            {
                                fill_SL_configuration(msg_p, &SLconfig, i, j, RC.config_file_name);
                            }
                            else
                            {
                                printf("No SL configuration skipping it\n");
                            }
...
int check_plmn_identity(rrc_eNB_carrier_data_t *carrier, uint16_t mcc, uint16_t mnc, uint8_t mnc_digit_length)
{
    ...
    if(mcc !=
            ...
            (*plmn_Identity_info->plmn_Identity.mcc->list.array[2]))
    {
        return(0);
    }

    // check that mnc digit length is different and return failure if so
    if(mnc_digit_length != plmn_Identity_info->plmn_Identity.mnc.list.count)
    {
        return 0;
    }
    ...
    if(mnc_digit_length == 2 &&
            (mnc !=
             ...
             (*plmn_Identity_info->plmn_Identity.mnc.list.array[1])))
    {
        return(0);
    }
            ...
            (mnc !=
             ...
             (*plmn_Identity_info->plmn_Identity.mnc.list.array[2])))
    {
        return(0);
    }
...
void extract_and_decode_SI(int inst, int si_ind, uint8_t *si_container, int si_container_length)
{
    ...
    {
        AssertFatal(1 == 0, "No SI messages\n");
    }
}
