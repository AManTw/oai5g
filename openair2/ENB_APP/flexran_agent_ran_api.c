...
{
    if(!rrc_is_present(mod_id) || !RC.rrc[mod_id]->carrier[cc_id].sib1)
    {
        return 0;
    }
...
LTE_TimeAlignmentTimer_t  flexran_get_time_alignment_timer(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.mac_MainConfig)
    {
        return -1;
    }
...
Protocol__FlexMeasGapConfigPattern flexran_get_meas_gap_config(mid_t mod_id, rnti_t rnti)
{
    ...
    if(ue_context_p->ue_context.measGapConfig->present != LTE_MeasGapConfig_PR_setup)
    {
        return -1;
    }
...
int flexran_get_half_duplex(mid_t mod_id, rnti_t rnti)
{
    ...
    {
        if(bands->list.array[i]->halfDuplex > 0)
        {
            return 1;
        }
    }
...
int flexran_get_intra_sf_hopping(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.UE_Capability->featureGroupIndicators)
    {
        return -1;
    }
...
long flexran_get_ue_transmission_mode(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.physicalConfigDedicated)
    {
        return -1;
    }
...
BOOLEAN_t flexran_get_tti_bundling(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.mac_MainConfig->ul_SCH_Config)
    {
        return -1;
    }
...
long flexran_get_beta_offset_ack_index(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.physicalConfigDedicated->pusch_ConfigDedicated)
    {
        return -1;
    }
...
BOOLEAN_t flexran_get_simultaneous_ack_nack_cqi(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.physicalConfigDedicated->cqi_ReportConfig)
    {
        return -1;
    }
    if(!ue_context_p->ue_context.physicalConfigDedicated->cqi_ReportConfig->cqi_ReportPeriodic)
    {
        return -1;
    }
...
BOOLEAN_t flexran_get_ack_nack_simultaneous_trans(mid_t mod_id, uint8_t cc_id)
{
    ...
    if(!RC.rrc[mod_id]->carrier[cc_id].sib2)
    {
        return -1;
    }
...
long flexran_get_tdd_ack_nack_feedback_mode(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.physicalConfigDedicated->pucch_ConfigDedicated)
    {
        return -1;
    }
    if(!ue_context_p->ue_context.physicalConfigDedicated->pucch_ConfigDedicated->tdd_AckNackFeedbackMode)
    {
        return -1;
    }
...
long flexran_get_extended_bsr_size(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.mac_MainConfig->ext2)
    {
        return -1;
    }
    if(!ue_context_p->ue_context.mac_MainConfig->ext2->mac_MainConfig_v1020)
    {
        return -1;
    }
...
long flexran_get_rrc_neigh_phy_cell_id(mid_t mod_id, rnti_t rnti, long cell_id)
{
    ...
    if(!ue_context_p->ue_context.measResults->measResultNeighCells->choice.measResultListEUTRA.list.array[cell_id])
    {
        return -1;
    }
...
long flexran_get_rrc_ofp(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.measurement_info)
    {
        return -1;
    }
...
long flexran_get_rrc_per_event_maxReportCells(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.measurement_info->events)
    {
        return -1;
    }
    if(!ue_context_p->ue_context.measurement_info->events->per_event)
    {
        return -1;
    }
...
long flexran_get_rrc_a3_event_hysteresis(mid_t mod_id, rnti_t rnti)
{
    ...
    if(!ue_context_p->ue_context.measurement_info->events->a3_event)
    {
        return -1;
    }
...
int flexran_set_rrc_ofp(mid_t mod_id, rnti_t rnti, long offsetFreq)
{
    ...
    if(!((offsetFreq >= -15) && (offsetFreq <= 15)))
    {
        return -1;
    }
...
int flexran_set_rrc_per_event_maxReportCells(mid_t mod_id, rnti_t rnti, long maxReportCells)
{
    ...
    if(!((maxReportCells >= 1) && (maxReportCells <= 8)))
    {
        return -1;
    }
...
int flexran_set_rrc_a3_event_reportOnLeave(mid_t mod_id, rnti_t rnti, int reportOnLeave)
{
    ...
    if(!((reportOnLeave == 0) || (reportOnLeave == 1)))
    {
        return -1;
    }
...
int flexran_set_x2_ho_net_control(mid_t mod_id, int x2_ho_net_control)
{
    ...
    if(!((x2_ho_net_control == 0) || (x2_ho_net_control == 1)))
    {
        return -1;
    }
...
void flexran_set_ue_dl_slice_idx(mid_t mod_id, mid_t ue_id, int slice_idx)
{
    ...
    if(flexran_get_mac_ue_crnti(mod_id, ue_id) == NOT_A_RNTI)
    {
        return;
    }
...
int flexran_remove_dl_slice(mid_t mod_id, int slice_idx)
{
    ...
    if(sli->dl[slice_idx].sched_name)
    {
        free(sli->dl[slice_idx].sched_name);
    }
    --sli->n_dl;
...
int flexran_remove_ul_slice(mid_t mod_id, int slice_idx)
{
    ...
    if(sli->ul[slice_idx].sched_name)
    {
        free(sli->ul[slice_idx].sched_name);
    }
    --sli->n_ul;
    /* move last element to the position of the removed one */
