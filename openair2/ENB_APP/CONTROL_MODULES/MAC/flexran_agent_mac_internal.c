...
void flexran_agent_read_slice_dl_config(mid_t mod_id, int slice_idx, Protocol__FlexDlSlice *dl_slice)
{
    ...
    if(dl_slice->n_sorting < 1)
    {
        dl_slice->sorting = NULL;
    }
    ...
    else
    {
        ...
        if(verified)
        {
            for(int i = 0; i < sup->n_dl; i++)
            {
                ...
                if(!dls)
                {
                    dls = create_new_dl_slice(mod_id, sup->dl[i]->id);
                }
    ...
    else
    {
        ...
        if(verified)
        {
            for(int i = 0; i < sup->n_ul; i++)
            {
                ...
                if(!uls)
                {
                    uls = create_new_ul_slice(mod_id, sup->ul[i]->id);
                }
