...
static void get_options(void)
{
    ...
    if((cmdline_uemodeparams[CMDLINE_CALIBUERX_IDX].paramflags &  PARAMFLAG_PARAMSET) != 0)
    {
        mode = rx_calib_ue;
    }

    if((cmdline_uemodeparams[CMDLINE_CALIBUERXMED_IDX].paramflags &  PARAMFLAG_PARAMSET) != 0)
    {
        mode = rx_calib_ue_med;
    }

    if((cmdline_uemodeparams[CMDLINE_CALIBUERXBYP_IDX].paramflags &  PARAMFLAG_PARAMSET) != 0)
    {
        mode = rx_calib_ue_byp;
    }

    if(cmdline_uemodeparams[CMDLINE_DEBUGUEPRACH_IDX].uptr)
        if(*(cmdline_uemodeparams[CMDLINE_DEBUGUEPRACH_IDX].uptr) > 0)
        {
            mode = debug_prach;
        }

    if(cmdline_uemodeparams[CMDLINE_NOL2CONNECT_IDX].uptr)
        if(*(cmdline_uemodeparams[CMDLINE_NOL2CONNECT_IDX].uptr) > 0)
        {
            mode = no_L2_connect;
        }

    if(cmdline_uemodeparams[CMDLINE_CALIBPRACHTX_IDX].uptr)
        if(*(cmdline_uemodeparams[CMDLINE_CALIBPRACHTX_IDX].uptr) > 0)
        {
            mode = calib_prach_tx;
        }
...
#  define PACKAGE_VERSION "UNKNOWN-EXPERIMENTAL"
    ...
    {
        init_openair0(frame_parms[0], (int)rx_gain[0][0]);
    }
