...
static void *L1_thread_tx(void *param)
{
    ...
    while(!oai_exit)
    {
        ...
        if(wait_on_condition(&proc->mutex, &proc->cond, &proc->instance_cnt, thread_name) < 0)
        {
            break;
        }
...
static void *L1_thread(void *param)
{
    ...
    while(!oai_exit)
    {
        ...
        if(eNB->CC_id == 0)
        {
            if(rxtx(eNB, proc, thread_name) < 0)
            {
                break;
            }
        }
        ...
        if(NFAPI_MODE != NFAPI_MODE_VNF)
        {
            ...
            {
                wakeup_tx(eNB, proc->frame_rx, proc->subframe_rx, proc->frame_tx, proc->subframe_tx, proc->timestamp_tx);
            }
        ...
        if(release_thread(&proc->mutex, &proc->instance_cnt, thread_name) < 0)
        {
            break;
        }
...
static void *eNB_thread_prach(void *param)
{
    ...
    while(!oai_exit)
    {
        if(wait_on_condition(&proc->mutex_prach, &proc->cond_prach, &proc->instance_cnt_prach, "eNB_prach_thread") < 0)
        {
            break;
        }
        ...
        if(release_thread(&proc->mutex_prach, &proc->instance_cnt_prach, "eNB_prach_thread") < 0)
        {
            break;
        }
    }
...
static void *eNB_thread_prach_br(void *param)
{
    ...
    while(!oai_exit)
    {
        if(wait_on_condition(&proc->mutex_prach_br, &proc->cond_prach_br, &proc->instance_cnt_prach_br, "eNB_prach_thread_br") < 0)
        {
            break;
        }
        ...
        if(release_thread(&proc->mutex_prach_br, &proc->instance_cnt_prach_br, "eNB_prach_thread_br") < 0)
        {
            break;
        }
    }
...
void init_eNB(int single_thread_flag, int wait_for_sync)
{
    ...
    if(RC.eNB == NULL)
    {
        RC.eNB = (PHY_VARS_eNB ** *) malloc(RC.nb_L1_inst * sizeof(PHY_VARS_eNB **));
    }
    ...
    for(inst = 0; inst < RC.nb_L1_inst; inst++)
    {
        if(RC.eNB[inst] == NULL)
        {
            RC.eNB[inst] = (PHY_VARS_eNB **) malloc(RC.nb_CC[inst] * sizeof(PHY_VARS_eNB *));
        }
        ...
        for(CC_id = 0; CC_id < RC.nb_L1_CC[inst]; CC_id++)
        {
            ...
            {
                RC.eNB[inst][CC_id] = (PHY_VARS_eNB *) malloc(sizeof(PHY_VARS_eNB));
            }
