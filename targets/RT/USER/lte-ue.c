...
void init_UE(int nb_inst, int eMBMS_active, int uecap_xer_in, int timing_correction, int phy_test, int UE_scan, int UE_scan_carrier, runmode_t mode, int rxgain, int txpowermax, LTE_DL_FRAME_PARMS *fp0)
{
    ...
    {
        PHY_vars_UE_g = (PHY_VARS_UE ***)calloc(1 + nb_inst, sizeof(PHY_VARS_UE **));
    }

    for(inst = 0; inst < nb_inst; inst++)
    {
        if(PHY_vars_UE_g[inst] == NULL)
        {
            PHY_vars_UE_g[inst] = (PHY_VARS_UE **)calloc(1 + MAX_NUM_CCs, sizeof(PHY_VARS_UE *));
        }
...
void ue_stub_rx_handler(unsigned int num_bytes, char *rx_buffer)
{
    ...
    switch(((UE_tport_header_t *)rx_buffer)->packet_type)
    {
        ...
        case SLSCH:
            ...
            printf("SLSCH:");
            for(int i = 0; i < sizeof(SLSCH_t); i++)
            {
                printf("%x ", ((uint8_t *)slsch)[i]);
            }
            printf("\n");
        ...
        case SLDCH:
            ...
            printf("SLDCH:");
            for(int i = 0; i < sizeof(SLDCH_t); i++)
            {
                printf("%x ", ((uint8_t *)sldch)[i]);
            }
            printf("\n");
...
static void *timer_thread(void *param)
{
    ...
    if(nfapi_mode == 4)
    {
        ...
        {
            LOG_I(PHY, "Running with internal timer\n");
        }
    }
