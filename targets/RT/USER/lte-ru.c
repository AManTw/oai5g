...
void *ru_thread_prach_br(void *param)
{
    ...
    while(!oai_exit)
    {
        if(wait_on_condition(&proc->mutex_prach_br, &proc->cond_prach_br, &proc->instance_cnt_prach_br, "ru_prach_thread_br") < 0)
        {
            break;
        }
        ...
        if(release_thread(&proc->mutex_prach_br, &proc->instance_cnt_prach_br, "ru_prach_thread_br") < 0)
        {
            break;
        }
    }
...
void *ru_thread(void *param)
{
    ...
    while(!oai_exit)
    {
        ...
        if(!(get_softmodem_params()->emulate_rf))
        {
            ...
            {
                AssertFatal(ru->state == RU_SYNC || ru->state == RU_RUN || ru->state == RU_CHECK_SYNC, "ru %d state = %s != RU_SYNC or RU_RUN or RU_CHECK_SYNC\n", ru->idx, ru_states[ru->state]);
            }
...
static void *eNB_thread_phy_tx(void *param)
{
    ...
    while(!oai_exit)
    {
        ...
        if(wait_on_condition(&proc->mutex_phy_tx, &proc->cond_phy_tx, &proc->instance_cnt_phy_tx, "eNB_phy_tx_thread") < 0)
        {
            break;
        }
        ...
        if(release_thread(&proc->mutex_phy_tx, &proc->instance_cnt_phy_tx, "eNB_thread_phy_tx") < 0)
        {
            break;
        }
...
static void *rf_tx(void *param)
{
    ...
    while(!oai_exit)
    {
        ...
        if(wait_on_condition(&proc->mutex_rf_tx, &proc->cond_rf_tx, &proc->instance_cnt_rf_tx, "rf_tx_thread") < 0)
        {
            break;
        }
        ...
        if(release_thread(&proc->mutex_rf_tx, &proc->instance_cnt_rf_tx, "rf_tx") < 0)
        {
            break;
        }
        ...
        for(j = 0; j < RC.nb_RU; j++)
        {
            ...
            if(strcmp(*(RUParamList.paramarray[j][RU_LOCAL_RF_IDX].strptr), "yes") == 0)
            {
                ...
                else
                {
                    ...
                    if(strcmp(*(RUParamList.paramarray[j][RU_OTA_SYNC_ENABLE_IDX].strptr), "yes") == 0)
                    {
                        RC.ru[j]->ota_sync_enable = 1;
                    }
                    else
                    {
                        RC.ru[j]->ota_sync_enable = 0;
                    }
                }
