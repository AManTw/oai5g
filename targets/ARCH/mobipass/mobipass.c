...
{
    if(ts < mobi->synch_time_last_ts)
    {
        mobi->synch_time_mega_ts++;
    }
...
{
    if(recv(mobi->sock, b, 14 + 14 + 1280, 0) != 14 + 14 + 1280)
    {
        perror("recv");
        exit(1);
    }
                    ...
                    unsigned char *packet)
{
    ...
    if(send(mobi->sock, packet, 14 + 14 + 1280, 0) != 14 + 14 + 1280)
    {
        perror("send");
        exit(1);
    }
...
void init_mobipass(mobipass_state_t *mobi)
{
    ...
    if(ioctl(mobi->sock, SIOCGIFINDEX, &if_index) < 0)
    {
        perror("SIOCGIFINDEX");
        exit(1);
    }
