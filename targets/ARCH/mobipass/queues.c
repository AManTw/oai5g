...
{
    queue_state_t *qstate = _qstate;
    if(pthread_mutex_lock(&qstate->to_mobipass.mutex))
    {
        abort();
    }
    while(qstate->to_mobipass.len == 0)
    {
        if(pthread_cond_wait(&qstate->to_mobipass.cond, &qstate->to_mobipass.mutex))
        {
            abort();
        }
    }
    ...
    if(pthread_mutex_unlock(&qstate->to_mobipass.mutex))
    {
        abort();
    }
}
...
static int cmp_timestamps(uint32_t a, uint32_t b, int samples_per_1024_frames)
{
    ...
    if(a < b)
    {
        if(b - a > samples_per_1024_frames / 2)
        {
            return 1;
        }
        return -1;
    }
    if(a - b > samples_per_1024_frames / 2)
    {
        return -1;
    }
    return 1;
...
static void get_sample_from_mobipass(queue_state_t *qstate, char *I, char *Q, uint32_t timestamp)
{
    ...
    while(qstate->from_mobipass.len)
    {
        ...
        if(cmp_timestamps(timestamp, packet_timestamp, qstate->samples_per_1024_frames) < 0)
        {
            goto nodata;
        }
        if(cmp_timestamps(timestamp, (packet_timestamp + 640) % qstate->samples_per_1024_frames, qstate->samples_per_1024_frames) < 0)
        {
            break;
        }
    ...
    {
        timestamp += qstate->samples_per_1024_frames;
    }
...
static void wait_for_data(pthread_cond_t *cond, pthread_mutex_t *mutex, int delay_us)
{
    ...
    if(target.tv_nsec >= 1000 * 1000 * 1000)
    {
        target.tv_nsec -= 1000 * 1000 * 1000;
        target.tv_sec++;
    }
    int err = pthread_cond_timedwait(cond, mutex, &target);
    if(err != 0 && err != ETIMEDOUT)
    {
        printf("mobipass: ERROR: pthread_cond_timedwait: err (%d) %s\n", err, strerror(err));
        abort();
    }
}
...
void dequeue_from_mobipass(void *_qstate, uint32_t timestamp, void *data)
{
    ...
    if(pthread_mutex_lock(&qstate->from_mobipass.mutex))
    {
        abort();
    }
    ...
    if(pthread_mutex_unlock(&qstate->from_mobipass.mutex))
    {
        abort();
    }
...
void *init_queues(int samples_per_1024_frames)
{
    ...
    if(pthread_mutex_init(&q->to_mobipass.mutex, NULL))
    {
        abort();
    }
    if(pthread_mutex_init(&q->from_mobipass.mutex, NULL))
    {
        abort();
    }
    if(pthread_cond_init(&q->to_mobipass.cond, NULL))
    {
        abort();
    }
    if(pthread_cond_init(&q->from_mobipass.cond, NULL))
    {
        abort();
    }
