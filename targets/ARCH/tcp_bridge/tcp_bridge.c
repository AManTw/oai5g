...
void get_buffer(tcp_bridge_state_t *t, void *data, int size, unsigned long timestamp)
{
    ...
    if(t->blen == 0)
    {
        printf("tcp_bridge: buffer empty\n");
        exit(1);
    }
    if(size > 30720 * 4)
    {
    ...
    pos = t->bstart;
    if(size != t->b[pos].size)
    {
        printf("tcp_bridge: bad size\n");
        exit(1);
    }
...
{
    if(fullread(sock, &b->timestamp, sizeof(b->timestamp)) != sizeof(b->timestamp))
    {
        goto err;
    }
    if(fullread(sock, &b->size, sizeof(b->size)) != sizeof(b->size))
    {
        goto err;
    }
    if(fullread(sock, b->data, b->size) != b->size)
    {
        goto err;
    }
    return;
...
{
    if(fullwrite(sock, &b->timestamp, sizeof(b->timestamp)) != sizeof(b->timestamp))
    {
        goto err;
    }
    if(fullwrite(sock, &b->size, sizeof(b->size)) != sizeof(b->size))
    {
        goto err;
    }
    if(fullwrite(sock, b->data, b->size) != b->size)
    {
        goto err;
    }
    return;
...
int tcp_bridge_write(openair0_device *device, openair0_timestamp timestamp, void **buff, int nsamps, int cc, int flags)
{
    ...
    if(i == t->blen)
    {
        ...
        t->blen++;
        if(t->b[nextpos].timestamp != timestamp)
        {
            abort();
        }
    }
...
int tcp_bridge_read(openair0_device *device, openair0_timestamp *timestamp, void **buff, int nsamps, int cc)
{
    ...
    if(b->timestamp != t->read_timestamp)
    {
        abort();
    }
    if(b->size != nsamps * 4)
