...
asn_comp_rval_t *et_s1ap_is_matching(et_s1ap_t *const s1ap1, et_s1ap_t *const s1ap2, const uint32_t constraints)
{
    ...
    if(s1ap1->pdu.present != s1ap2->pdu.present)
    {
        rv  = calloc(1, sizeof(asn_comp_rval_t));
        rv->err_code = ET_ERROR_MATCH_PACKET_S1AP_PRESENT;
        return rv;
    }
    switch(s1ap1->pdu.present)
    {
        ...
        case  S1AP_PDU_PR_initiatingMessage:
            ...
            {
                rv  = calloc(1, sizeof(asn_comp_rval_t));
                rv->err_code = ET_ERROR_MATCH_PACKET_S1AP_PROCEDURE_CODE;
                return rv;
            }
            if(s1ap1->pdu.choice.initiatingMessage.criticality != s1ap2->pdu.choice.initiatingMessage.criticality)
            {
                rv  = calloc(1, sizeof(asn_comp_rval_t));
                rv->err_code = ET_ERROR_MATCH_PACKET_S1AP_CRITICALITY;
                return rv;
            }
            break;
    ...
    {
        if(memcmp((void *)s1ap1->binary_stream, (void *)s1ap2->binary_stream, s1ap1->binary_stream_allocated_size) ==  0)
        {
            return NULL;
        }
    }
