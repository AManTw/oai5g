...
int decode_pdn_disconnect_reject(pdn_disconnect_reject_msg *pdn_disconnect_reject, uint8_t *buffer, uint32_t len)
{
    uint32_t decoded = 0;
    ...
    while(len - decoded > 0)
    {
        ...
        switch(ieiDecoded)
        {
            case PDN_DISCONNECT_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
                if((decoded_result =
                            decode_protocol_configuration_options(&pdn_disconnect_reject->protocolconfigurationoptions,
