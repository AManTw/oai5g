...
int decode_bearer_resource_modification_reject(bearer_resource_modification_reject_msg *bearer_resource_modification_reject, uint8_t *buffer, uint32_t len)
{
    uint32_t decoded = 0;
    ...
    while(len - decoded > 0)
    {
        ...
        switch(ieiDecoded)
        {
            case BEARER_RESOURCE_MODIFICATION_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
                if((decoded_result =
                            decode_protocol_configuration_options(&bearer_resource_modification_reject->protocolconfigurationoptions,
...
int encode_bearer_resource_modification_reject(bearer_resource_modification_reject_msg *bearer_resource_modification_reject, uint8_t *buffer, uint32_t len)
{
    int encoded = 0;
    int encode_result = 0;
    ...
    {
        if((encode_result =
                    encode_protocol_configuration_options(&bearer_resource_modification_reject->protocolconfigurationoptions,
                            BEARER_RESOURCE_MODIFICATION_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_IEI,
