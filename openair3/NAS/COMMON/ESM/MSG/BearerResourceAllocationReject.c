...
int decode_bearer_resource_allocation_reject(bearer_resource_allocation_reject_msg *bearer_resource_allocation_reject, uint8_t *buffer, uint32_t len)
{
    uint32_t decoded = 0;
    ...
    while(len - decoded > 0)
    {
        ...
        switch(ieiDecoded)
        {
            case BEARER_RESOURCE_ALLOCATION_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
                if((decoded_result =
                            decode_protocol_configuration_options(&bearer_resource_allocation_reject->protocolconfigurationoptions,
